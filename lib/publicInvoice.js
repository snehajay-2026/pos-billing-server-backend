...
function pickTotalsFromItems(items) {
  ...
  for (const it of items) {
    if (!it || typeof it !== "object") continue;
    const price = num(it.price != null ? it.price : it.rate);
    let qty = 1;
    if (it.hours != null) qty = num(it.hours);
    else if (it.qty != null) qty = num(it.qty);
    else if (it.qtyKg != null) qty = num(it.qtyKg);
    else if (it.units != null) qty = num(it.units);
...
function pickServiceMetaFromItems(items) {
  if (!Array.isArray(items)) return {};
  const clean = (value) => {
  ...
  for (const item of items) {
    ...
    if (!meta) continue;
    const result = {
      technician: clean(meta.technician),
      jobRef: clean(meta.jobRef),
      serviceFrom: clean(meta.serviceFrom),
      serviceTo: clean(meta.serviceTo),
      remarks: clean(meta.remarks),
      gstRate: meta.gstRate != null ? Number(meta.gstRate) : null,
      discountPct: meta.discountPct != null ? Number(meta.discountPct) : null,
      discountAmt: meta.discountAmt != null ? Number(meta.discountAmt) : null,
    };
    if (Object.values(result).some((value) => value !== null && value !== undefined && value !== "")) {
      return result;
    }
  }
  return {};
...
// Payment mode fallback for legacy rows.
...
// Walk items[].meta (BEFORE the meta strip below) and lift the
// industry-specific invoice template metadata the cashier picked at
// billing time. The frontend `ServiceBilling.jsx` mirrors three
// fields onto items[0].meta:
//
//   - industry      — one of the 16 ids in the shared template
//                     registry (consulting, manufacturing, …)
//   - templateId    — the per-invoice renderer family + sections
//                     (consulting-modern, manufacturing-traditional, …)
//   - fields        — the cashier-typed per-industry data (PO number,
//                     engagement ref, etc.)
//
// The renderer chain (`resolveTemplate` + the family dispatch in
// `InvoiceView`/`PublicInvoiceView`) already reads these three
// fields from `items[0].meta`, but the public share link runs
// through `sanitizePublicInvoice` which strips the entire `meta`
// block. Without this hoist the public viewer would always fall
// back to the legacy `ServiceInvoice` / `MSMEInvoice` renderer,
// even when the cashier preview uses ModernA4 / TraditionalA4 /
// CondensedReceipt.
//
// Same "only fill missing values" pattern as the other hoists in
// this module — top-level values win, meta only fills the gap.
function pickTemplateMetaFromItems(items) {
  if (!Array.isArray(items)) return {};
  const clean = (value) => {
    if (value == null) return null;
    const text = String(value).trim();
  ...
  for (const item of items) {
    ...
    if (!meta) continue;
    const industry = clean(meta.industry);
    const templateId = clean(meta.templateId);
    // `fields` is an object, not a string. Accept it as-is when it's a
    // non-empty plain object — the renderer reads `fields[key]`
    // directly. Skip when it's missing, null, or a non-object shape so
    // a malformed legacy row never leaks a typed-array-as-fields bug.
    ...
    if (rawFields && typeof rawFields === "object" && !Array.isArray(rawFields)) {
      ...
      for (const [k, v] of Object.entries(rawFields)) {
        ...
        fields[String(k)] = typeof v === "string" ? v : String(v);
      }
      if (Object.keys(fields).length === 0) fields = null;
    }
    if (industry || templateId || fields) {
      return {
...
// Walk items[].meta (BEFORE the meta strip below) and pull Hotel
// Lodging fields that only live on the line item: the booked room
// number / name. Lodging bookings attach `roomName` / `roomId` to
// `items[0].meta` ([HotelLodgingPage.jsx -> "Room Booking - ..."]
// shape, see src/pages/HotelLodgingPage.jsx around the Quick Book
// path that fires `hotel_quick_book`); the dedicated invoices table
// has no column for the booked room. Without hoisting, the public
// sanitizer strips `meta` and the 80mm Thermal Public Invoice falls
// back to "—" for the Room Number even though the cashier's authed
// preview (which sees `meta`) shows the real room. We mirror the
// Dining hoist: lift the room identifier onto `hotelDetails.roomNumber`
// (which the renderer checks first) so the WhatsApp / Email share
// link renders the actual booked Room Number (e.g. "Room No: 105")
// instead of a dash.
//
// Preference order: meta.roomName (the display label cashier typed)
// > meta.roomId (the stable key) > nothing. We do NOT consult the
// hotel `rooms` slice here — the cashier may have booked a room that
// has since been renamed/deleted, so the line-item's own snapshot
// (what the booking actually used) is the authoritative source.
function pickLodgingMetaFromItems(items) {
  if (!Array.isArray(items)) return {};
  const clean = (v) => {
    if (v == null) return null;
    const s = String(v).trim();
  ...
  for (const it of items) {
    ...
    const meta = it.meta && typeof it.meta === "object" ? it.meta : null;
    if (!meta) continue;
    const roomNumber = clean(meta.roomName) || clean(meta.roomId);
...
async function sanitizePublicInvoice(row) {
  ...
  // Hoist safe Service Store display fields before stripping meta. These are
  // snapshots entered at billing time, not internal scope/auth data.
  if (rest.storeType === "service" || rest.storeType === "msme-service") {
    const serviceMeta = pickServiceMetaFromItems(rest.items);
    for (const key of ["technician", "jobRef", "serviceFrom", "serviceTo", "remarks"]) {
      if (!rest[key] && serviceMeta[key]) rest[key] = serviceMeta[key];
    }
    if (rest.gstRate == null && Number.isFinite(serviceMeta.gstRate)) rest.gstRate = serviceMeta.gstRate;
    if (rest.discountPct == null && Number.isFinite(serviceMeta.discountPct)) rest.discountPct = serviceMeta.discountPct;
    if (rest.discountAmt == null && Number.isFinite(serviceMeta.discountAmt)) rest.discountAmt = serviceMeta.discountAmt;

    // F9: industry-specific invoice template metadata (industry,
    // templateId, fields). These three keys are exactly what the
    // `resolveTemplate` helper on the frontend reads to pick the
    // ModernA4 / TraditionalA4 / CondensedReceipt renderer. Without
    // this hoist the meta-strip below would erase them and the
    // public share link would always render the legacy
    // ServiceInvoice / MSMEInvoice fallback. Only fills missing
    // values so a future /api/invoices/:invoiceNo response that
    // surfaces the metadata at the top level wins.
    const templateMeta = pickTemplateMetaFromItems(rest.items);
    if (!rest.industry && templateMeta.industry) rest.industry = templateMeta.industry;
    if (!rest.templateId && templateMeta.templateId) rest.templateId = templateMeta.templateId;
    if (
      (!rest.fields || (typeof rest.fields === "object" && Object.keys(rest.fields).length === 0)) &&
      templateMeta.fields
    ) {
      rest.fields = templateMeta.fields;
    }
  }
  ...
  //
  // For Hotel Lodging we additionally hoist the booked Room Number —
  // the renderer (HotelThermalReceipt `LodgingMeta`) reads
  // `hotelDetails.roomNumber` first and `items[0].meta.roomName` /
  // `items[0].meta.roomId` second, but the meta-strip below removes
  // those per-item fallbacks, so without this hoist the 80mm Thermal
  // Public Invoice falls back to "—" for Room No even when the
  // cashier's authed preview shows the real room. Only fills missing
  // values — anything already on `hotelDetails.roomNumber` wins.
  if (rest.storeType === "hotel") {
    const fromDining = pickDiningMetaFromItems(rest.items);
    const fromLodging = pickLodgingMetaFromItems(rest.items);
    ...
    if (!rest.hotelDetails.roomNumber && fromLodging.roomNumber) {
      rest.hotelDetails.roomNumber = fromLodging.roomNumber;
    }
  }
...
module.exports = {
  ...
  // Exported for tests + reuse — hoist the booked Room Number off a
  // Lodging line item's `meta` block. Used by sanitizePublicInvoice
  // for the WhatsApp / Email share link so the 80mm Thermal Public
  // Invoice renders the real Room No instead of "—".
  pickLodgingMetaFromItems,
  pickServiceMetaFromItems,
  pickTotalsFromItems,
  // F9: hoist the per-invoice industry template metadata (industry,
  // templateId, fields) off a Service / MSME-Service line item's
  // `meta` block. Used by sanitizePublicInvoice so the public share
  // link renders the same ModernA4 / TraditionalA4 / CondensedReceipt
  // layout the cashier previewed.
