...
// resolve: turn a stored relative path (e.g. "uploads/products/foo.jpg")
// or a bare filename into an absolute filesystem path. Returns null for
// any input that tries to escape UPLOAD_ROOT (path traversal guard).
const resolveSafe = (stored) => {
  if (!stored) return null;
  const abs = path.isAbsolute(stored)
    ? stored
    : path.join(UPLOAD_ROOT, stored);
